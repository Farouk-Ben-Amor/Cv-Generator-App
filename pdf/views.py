...
def accept(request):
    if request.method=="POST":
        ...
        if not all([name, email, phone, summary, degree, school, university, previous_work, skills]):
            # If any of the form fields are empty, return an error message
            error_message = "All form fields are required."
            return render(request, 'pdf/accept.html', {'error_message': error_message})
